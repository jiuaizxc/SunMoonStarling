...
package morn.core.components{
	...
	public class Clip extends Component implements IClip {
		...
		protected function changeClip():void{
			var texture:Texture = App.asset.getTexture(StringUtils.assetsName(_url));
			if (texture) loadComplete(_url, texture);
		}
		
		protected function loadComplete(url:String, bmd:Texture):void {
			if (url == _url && bmd) {
				if (!isNaN(_clipWidth)) {
				...
				clips = BitmapUtils.getClips(StringUtils.assetsName(url), _clipX, _clipY);
			}
		}
